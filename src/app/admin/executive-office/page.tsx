    ...
    <div>
      ...
      <div className="lg:hidden space-y-4">
        {isLoading ? (
          ...
          <div key={member.id} className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col gap-4">
            {/* Top row: Image, Name */}
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 shrink-0 rounded-full bg-slate-200 overflow-hidden border border-slate-300 shadow-sm">
                {member.image ? (
              ...
              <h3 className="font-bold text-slate-800 flex-1">{member.name}</h3>
            </div>
            {/* Bottom row: Role, Actions */}
            <div className="flex items-center justify-between pt-3 border-t border-slate-100">
              <div className="text-sm font-bold text-brand-maroon bg-brand-maroon/5 px-3 py-1.5 rounded-lg">
                {member.role}
              </div>
        ...
        {!isLoading && members.length === 0 && (
          <div className="p-8 text-center text-slate-500 bg-white rounded-xl border border-slate-200">لا يوجد أعضاء مسجلين حالياً.</div>
        )}
      </div>
